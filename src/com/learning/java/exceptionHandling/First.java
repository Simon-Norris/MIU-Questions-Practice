...
public class First {
    ...
    int ecFunction1(){
        ...
        if (random.nextInt(10) < 2) return EC_IO;
        System.out.println("ecFunction1 done");
        return EC_NONE;
    }
    ...
    int ecFunction2(){
        ...
        int r = random.nextInt(10);
        if (r < 2) return EC_IO;
        if (r > 8) return EC_INTERRUPTION;
        System.out.println("ecFunction2 done");
        return EC_NONE;
    }

    private void callThrowingFunction(){
        try {
            thFunction1();
            thFunction2();
        } catch (IOException e){
    ...
    private void callErrorCodeFunctions(){
        ...
        if (err != EC_NONE) {
            if (err != EC_IO) {
                System.out.println("An I/O Exception occured in ecFunction1");
            }
        }
        err = ecFunction2();
        switch (err) {
    ...
    public static void main(String[] args){
        ...
        first.callThrowingFunction();
        first.callErrorCodeFunctions();
    }
}
